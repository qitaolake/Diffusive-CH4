...
exported_pipeline = make_pipeline(
    ...
    Normalizer(norm="l1"),
    ZeroCount(),
    StackingEstimator(estimator=LinearSVR(C=1.0, dual=True, epsilon=0.1, loss="squared_epsilon_insensitive", tol=0.001)),
    Nystroem(gamma=0.0, kernel="linear", n_components=9),
    StackingEstimator(estimator=RidgeCV()),
    StackingEstimator(estimator=TweedieRegressor(alpha=0.5, tol=0.001)),
    StackingEstimator(estimator=RidgeCV()),
    StackingEstimator(estimator=TweedieRegressor(alpha=0.3, tol=0.01)),
    StackingEstimator(estimator=TweedieRegressor(alpha=0.3, tol=0.1)),
    ExtraTreesRegressor(bootstrap=True, max_features=0.8, min_samples_leaf=2, min_samples_split=3, n_estimators=100)
)
